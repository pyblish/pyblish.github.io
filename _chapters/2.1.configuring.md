...
> On Windows, naming files this way can be quite the hassle. Here's a hint; try naming the file ".pyblish.", with a leading dot.
...
By creating a user configuration and *overriding* the `paths` variable, we've stripped Pyblish of it's default path for plugins. If this is what you want, that's fine. Otherwise, include the path from the original config.
